# Hybrid weather source: NASA POWER variables + local TAMSAT rainfall NetCDF.
#
# TAMSAT (Univ. Reading) is a 4 km daily African rainfall product (1983-present).
# It carries rainfall only, so this backend mirrors the CHIRPS/MSWEP hybrids:
...
def _worker(args):
    ...
    rain = pd.Series(args["tamsat_rain"], dtype="float64")
    mapped = df["DATE"].map(rain)
    use = mapped.notna()
    df.loc[use, "RAIN"] = mapped[use].values
    tav, amp = _calc_tav(df), _calc_amp(df)
    header = (
        f"$WEATHER DATA: NASA-POWER + TAMSAT rain (Point ID: {pid})\n"
        f"@ INSI      LAT     LONG  ELEV   TAV   AMP REFHT WNDHT\n"
        f"  TMST {lat:8.4f} {lon:8.4f}   -99 {tav:5.1f} {amp:5.1f}   2.0   2.0\n"
...
def process_weather_tamsat(
    shapefile, start_year, end_year, output_dir,
    id_col, lat_col, lon_col, n_cores, log_file,
    tamsat_nc_dir: str,
) -> None:
    """Write DSSAT .WTH using NASA POWER plus local TAMSAT (Africa) rainfall."""
    ...
    path = find_nc_file(tamsat_nc_dir, ["tamsat", "rfe", "rain", "precip"])
    if not path:
        raise FileNotFoundError(f"TAMSAT rainfall NetCDF not found in {tamsat_nc_dir}")
    rain = extract_netcdf_series(path, ["rfe", "rain", "precip"], ids, lats, lons,
    ...
    for pid, lat, lon in zip(ids, lats, lons):
        tasks.append({"pid": pid, "lat": float(lat), "lon": float(lon),
                      ...
                      "end": f"{int(end_year)}1231", "tamsat_rain": rain.get(pid, {})})
    print(f"--- Starting NASA-POWER + TAMSAT Processing (Years: {start_year}-{end_year}) ---")
        ...
        for fut in as_completed([pool.submit(_worker, t) for t in tasks]):
            try:
                ...
                with open(log_file, "a") as lf:
                    lf.write(f"TAMSAT hybrid worker failed: {exc}\n")
    print(f"\nNASA-POWER + TAMSAT processing complete. Check '{output_dir}'.\n")
