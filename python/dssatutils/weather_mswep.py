# Hybrid weather source: NASA POWER variables + local MSWEP rainfall NetCDF.
...
def _worker(args):
    ...
    rain = pd.Series(args["mswep_rain"], dtype="float64")
    mapped = df["DATE"].map(rain)
    use = mapped.notna()
    df.loc[use, "RAIN"] = mapped[use].values
    tav, amp = _calc_tav(df), _calc_amp(df)
    header = (
        f"$WEATHER DATA: NASA-POWER + MSWEP rain (Point ID: {pid})\n"
        f"@ INSI      LAT     LONG  ELEV   TAV   AMP REFHT WNDHT\n"
        f"  MSWP {lat:8.4f} {lon:8.4f}   -99 {tav:5.1f} {amp:5.1f}   2.0   2.0\n"
...
def process_weather_mswep(
    shapefile, start_year, end_year, output_dir,
    id_col, lat_col, lon_col, n_cores, log_file,
    mswep_nc_dir: str,
) -> None:
    """Write DSSAT .WTH using NASA POWER plus local MSWEP precipitation."""
    ...
    path = find_nc_file(mswep_nc_dir, ["mswep", "precip", "pr"])
    if not path:
        raise FileNotFoundError(f"MSWEP precipitation NetCDF not found in {mswep_nc_dir}")
    rain = extract_netcdf_series(path, ["precip", "pr"], ids, lats, lons, int(start_year), int(end_year), "rain")
    tasks = []
    for pid, lat, lon in zip(ids, lats, lons):
        tasks.append({"pid": pid, "lat": float(lat), "lon": float(lon),
                      ...
                      "end": f"{int(end_year)}1231", "mswep_rain": rain.get(pid, {})})
    print(f"--- Starting NASA-POWER + MSWEP Processing (Years: {start_year}-{end_year}) ---")
        ...
        for fut in as_completed([pool.submit(_worker, t) for t in tasks]):
            try:
                ...
                with open(log_file, "a") as lf:
                    lf.write(f"MSWEP hybrid worker failed: {exc}\n")
    print(f"\nNASA-POWER + MSWEP processing complete. Check '{output_dir}'.\n")
