# Hybrid weather source: NASA-POWER variables + local APHRODITE (Asia) rainfall.
# APHRODITE core product is gauge-based daily precipitation for monsoon Asia /
# South & SE Asia / Himalaya / Middle East (0.25-0.5 deg, 1951-). Temperature/
# radiation/humidity/wind come from NASA-POWER (CHIRPS/MSWEP hybrid pattern).

process_weather_aphrodite <- function(shapefile, start_year, end_year, output_dir,
                                      id_col, lat_col, lon_col, n_cores, log_file,
                                      aphrodite_nc_dir) {
  message(sprintf("--- Starting NASA-POWER + APHRODITE Processing (Years: %d-%d) ---", start_year, end_year))
  ...
  p <- weather_find_nc_file(aphrodite_nc_dir, c("aphro", "precip", "rain", "pr"))
  if (is.na(p)) stop(sprintf("APHRODITE rainfall NetCDF not found in %s", aphrodite_nc_dir))
  ...
  message(sprintf("\nNASA-POWER + APHRODITE processing complete. Check '%s'.\n", output_dir))
}
