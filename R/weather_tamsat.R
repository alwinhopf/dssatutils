# Hybrid weather source: NASA-POWER variables + local TAMSAT (Africa) rainfall.
# TAMSAT is rainfall-only (4 km daily, 1983-); mirrors the CHIRPS/MSWEP hybrids.

process_weather_tamsat <- function(shapefile, start_year, end_year, output_dir,
                                   id_col, lat_col, lon_col, n_cores, log_file,
                                   tamsat_nc_dir) {
  message(sprintf("--- Starting NASA-POWER + TAMSAT Processing (Years: %d-%d) ---", start_year, end_year))
  ...
  p <- weather_find_nc_file(tamsat_nc_dir, c("tamsat", "rfe", "rain", "precip"))
  if (is.na(p)) stop(sprintf("TAMSAT rainfall NetCDF not found in %s", tamsat_nc_dir))
  ...
  message(sprintf("\nNASA-POWER + TAMSAT processing complete. Check '%s'.\n", output_dir))
}
